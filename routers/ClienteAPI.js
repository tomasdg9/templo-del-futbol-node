...
router.post('/register', (req, res) => {
  ...
  /* Esto tendria que ser una transaccion */

  pool.query('SELECT email FROM clientes WHERE email = $1',
    [email],
    (error, results) => {
      if (error) {
        throw error;
        ...
        if (results.rows.length == 0) {
          pool.query('INSERT INTO clientes (email, password, nombre, apellido) VALUES ($1, $2, $3, $4)',
            ...
            (error, results) => {
              ...
              pool.query('INSERT INTO tokenclientes (email, token) VALUES ($1, $2)',
                [email, token],
                (error) => {
                  if (error) {
                    throw error;
  ...
  pool.query(
    ...
    (error, results) => {
      ...
      if (results.rows.length > 0) {
        ...
        // La contrsaeña ya debe venir encriptada.
		pool.query(
...
// incluir en el metodo de arriba
